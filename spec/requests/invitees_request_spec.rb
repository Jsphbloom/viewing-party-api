...
RSpec.describe "invitees_controller", :request do
  ...
  describe "sad paths" do
    it "returns an error when viewing party ID is invalid" do
      @host = create(:user, id: 3)
      @user1 = create(:user, id: 5)
      @user2 = create(:user, id: 11)
      @user3 = create(:user, id: 7)
      ...
      post "/api/v1/viewing_parties/2/invitees", headers: headers, params: JSON.generate(params)
      expect(response).to have_http_status(:not_found)

      json = JSON.parse(response.body, symbolize_names: true)

      expect(json[:errors]).to eq(["Invalid viewing party ID"])
    end

    it "returns an error when user ID is invalid" do
      @host = create(:user, id: 3)
      @user1 = create(:user, id: 5)
      @user2 = create(:user, id: 11)
      @user3 = create(:user, id: 7)
      ...
      expect(json[:errors]).to eq(["Invalid user ID"])
